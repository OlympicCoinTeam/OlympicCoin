...
#if defined(NDEBUG)
#error "OlympicCoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uolympic) are considered zero fee (for relaying and mining)
...
// olympiccoin: total coin age spent in transaction, in the unit of coin-days.
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    // olympiccoin: track money supply and mint amount info
...
CBlockIndex* AddToBlockIndex(const CBlock& block)
{
    ...
    if (miPrev != mapBlockIndex.end()) {
        ...
        // olympiccoin: compute chain trust score
        pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

        // olympiccoin: compute stake entropy bit for stake modifier
        if (!pindexNew->SetStakeEntropyBit(pindexNew->GetStakeEntropyBit()))
            LogPrintf("AddToBlockIndex() : SetStakeEntropyBit() failed \n");

        // olympiccoin: record proof-of-stake hash value
        if (pindexNew->IsProofOfStake()) {
        ...
        // olympiccoin: compute stake modifier
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    // olympiccoin: check proof-of-stake
